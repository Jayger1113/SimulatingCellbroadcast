package com.example.testcellbroadcast.constants;

public class Channel4393Constants {

    public static final String TEST_CHANNEL_MESSAGE = "This is channel testing message";

    public static final int TEST_CHANNEL_4393 = 4393;

    public static final String TEST_PLMN = "00101";

    public static final int TEST_LAC = 18;

    public static final int TEST_CELL_ID = 18;

    public static final int TEST_GEO_SCOPE = 0;

    public static final int TEST_SERIAL_NUM = 394;

    public static final String TEST_LANGUAGE = "en";

    public static final int TEST_MESSAGE_FORMAT = -1;// TODO

    public static final int TEST_PRIORITY = 3;

    public static final int TEST_SUBSCRIPTION = 0;

    public static final int TEST_CMASINFO_MESSAGE_CLASS = -1;

    public static final int TEST_CMASINFO_CATEGORY = -1;

    public static final int TEST_CMASINFO_RESPONSETYPE = -1;

    public static final int TEST_CMASINFO_SERVERITY = -1;

    public static final int TEST_CMASINFO_URGENCY = -1;

    public static final int TEST_CMASINFO_CERTAINTY = -1;
}
